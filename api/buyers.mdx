---
title: "Buyers API — Coffee Buyer Profiles and Prices"
sidebarTitle: "Buyers"
description: "REST endpoints to create, retrieve, update, and delete coffee buyer (comprador) profiles in CoffePrice. Buyer profiles require admin approval to appear on the public map."
---
...
The accepted body fields are identical to those of `POST /api/comprador` (`nombreempresa`, `tipoempresa`, `municipio`, `direccion`, `telefono`, `horarioApertura`, `horarioCierre`, `descripcion`, `servicios`), plus `latitud` (number) and `longitud` (number) for manual coordinate overrides.
