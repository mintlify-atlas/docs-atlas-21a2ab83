---
title: "Coffee Prices API — Buyer Price Management"
sidebarTitle: "Prices"
description: "REST endpoints to list, create, update, and delete coffee buyer prices in CoffePrice. Public GET, authenticated POST/PUT/DELETE restricted to comprador and admin roles."
---
...
<ResponseField name="unidad" type="string">
  Unit of measure: `carga` for `pergamino_seco`, `especial`, and `organico`; `kg` for `pasilla`, `cacao`, and `limon`. Set automatically on save.
</ResponseField>
...
<ParamField body="preciocarga" type="number" required>
  Offered price. Must be a finite number greater than `0`. Represents price per carga for `pergamino_seco`, `especial`, and `organico`; price per kg for `pasilla`, `cacao`, and `limon`.
</ParamField>
...
If the cache is populated and fresh, the response is returned immediately. If the cache is stale, a background refresh is triggered and the last known value is returned instantly. If the cache is empty and a refresh is in progress, the server responds with HTTP `202`. If no price can be obtained at all, the server responds with HTTP `503`.
