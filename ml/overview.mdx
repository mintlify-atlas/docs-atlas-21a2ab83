...
<CardGroup cols={2}>
  ...
  <Card title="Hybrid XGBoost" icon="microchip">
    Prophet's forecast plus an XGBoost model trained on the 40-feature residual. Captures non-linear interactions between lag prices, moving averages, and market signals. Ensemble weight: **0.092**.
  </Card>
</CardGroup>
...
```
signal_strength = (0.65 × |kc_change_pct| / 1.2) + (0.35 × |trm_change_pct| / 0.8)
                  clamped to [0, 1]
```
...
<Steps>
  ...
  <Step title="Feature Engineering">
    `pipeline_fnc_hibrido.py` builds the daily base dataframe. It merges FNC, KC, TRM, and external variables; adds formula columns; adds calendar features; and constructs a supervised frame with lag features, moving averages, volatility, and return rates — 40 features total.
  </Step>
